...
useEffect(() => {
  ...
  return (
    <div>
    <SingleCitation case={_case} type={"caselaw"} bookmarks={bookmark?.includes(_case?.Citation_id)} action={props.action}/>
    <div className="card" id="accordion">
    {cases && cases?.length <= 1?<div className="card-header results_length m-0" id="headingOne">This Case Law is cited in {cases.length} cases 
    <button className="btn btn-link" data-toggle="collapse" data-target="#collapseOne" aria-expanded="true" aria-controls="collapseOne"><AiFillCaretDown/></button>
    </div>:
    cases && cases?.length > 1?<div className="card-header results_length m-0" id="headingOne">This Case Law is cited in {cases.length} cases 
        <button className="btn-collapse" data-toggle="collapse" data-target="#collapseOne" aria-expanded="false" aria-controls="collapseOne"><AiFillCaretDown/></button>
    </div>:null}
    <div id="collapseOne" className="collapse" aria-labelledby="headingOne" data-parent="#accordion">
    {cases && cases?.length >0?
cases.map((obj)=>{
return <CitationBox case={obj} type={"caselaw"} bookmarks={bookmark?.includes(obj.Citation_id)} action1={openRefCases} action2={openRefStatutes}/>
}):null}
</div>
</div>
    </div>
